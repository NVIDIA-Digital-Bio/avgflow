...
def main(args, config):
    ...
    chembl_ids_list = smiles_df['chembl_id'].tolist()
    ...
    # smiles_df = smiles_df.iloc[36925:]
    # total_num_confs_in_csv = sum(smiles_df['num_confs'].tolist())
    print('Generating %d conformers for %d molecules'%(total_num_confs_in_csv, len(smiles_df)))
    ...
    for i, row in smiles_df.iterrows():
        ...
        dst = os.path.join(dst_dir, f'{chembl_id}.pkl')
        pickle.dump(
            output_dict, 
            open(dst, 'wb')
        )
