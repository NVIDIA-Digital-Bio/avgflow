...
from dataloader.jnp_dataloader import DataLoader
from flow_matcher.flow_matcher import FlowMatcher
from utils.model_utils import tree_stack, load_most_recent_ckpt_for_restart_cluster, dump_ckpt, set_lr_scheduler_configs
import optax
import wandb, compress_pickle, signal, pickle
...
def main(config):
    ...
    dataloader = DataLoader(
        [d['features'] for d in train_data], [d['conformers'] for d in train_data], 
        max_n=config['model']['max_n'], batchsize=config['model']['batchsize'],
        shuffle=True, drop_last=False
    )
    print('Loaded %d graphs for training'%len(train_data[0]))
    val_loader = DataLoader(
        [d['features'] for d in val_data], [d['conformers'] for d in val_data],
        max_n=config['model']['max_n'], batchsize=config['model']['batchsize'],
    ...
    # learning_rate = config['flow_matcher']['learning_rate']
    ...
    if not run_name:
        run_name = '%s_toydata_%dGPU_maxn%d_%s_%s'%(
            size_in_million, 
            device_count,
            config['model']['max_n'],
            flow_matcher.loss_mode,
            flow_matcher.time_schedule
        )
